...
class JobProfileSection extends Component {
  ...
  getJobDetails = async () => {
    ...
    const {salaryRange, employmentType, search} = this.state
    const url = `https://apis.ccbp.in/jobs?employment_type=${employmentType.join()}&minimum_package=${salaryRange}&search=${search}`
    const options = {
      headers: {
